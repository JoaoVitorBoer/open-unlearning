...
# PY
# )}
MASTER_PORT=12345  # Simulated fixed port for dry run
echo "Master Port (simulated): ${MASTER_PORT}"
echo "DRY RUN: printing commands only (no training or evaluation executed)."
...
for data_split in "${DATA_SPLITS[@]}"; do
  ...
  for trainer in "${TRAINERS[@]}"; do
    ...
    for adapter in "${ADAPTERS[@]}"; do
      ...
      echo
      echo "  Adapter: ${adapter_tag}"
      echo "  Task: ${task_name}"
      echo "  Train output: ${train_output_dir}"

      train_cmd=(
        CUDA_VISIBLE_DEVICES=0,1,2
        accelerate launch --config_file configs/accelerate/default_config.yaml --num_processes=3 --main_process_port "${MASTER_PORT}"
